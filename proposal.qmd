...
### 2. **Crime Trends Analysis Dashboard**

(Done by: Yee Kai)

A comprehensive dashboard to compare crime trends between different states and over various time periods. By filtering the dashboard by year and crime type, users can delve into specific crime patterns and view potential factors influencing these trends.

### 3. **State Comparison**

(Done by: Yee Kai)

Assess the relative crime rates of different states by viewing the total sum or mean of crimes for each state, compared to the national average. By filtering by year, state, and crime type, users can observe the crime patterns across West Malaysia.

### 4. **Global Spatial Autocorrelation (Moran's I and/or Geary's C)**

(Done by: Jenna)

Global Spatial Autocorrelation will be represented by a choropleth map showing **Moran's I or Geary's C values** for different crime types, with high positive values indicating clustering of similar values and high negative values indicating dispersion of dissimilar values.

### 5. **Local Spatial Autocorrelation (Local Moran's I - LISA Map)**

(Done by: Jenna)

Local Spatial Autocorrelation will be examined using Local Moran's I, visualized through a LISA map. This highlights statistically significant clusters of high and low values.

### 6. **Hot Cold Spot Analysis**

(Done by: Jenna)

Allows users to filter the map by hotspots or coldspots and customize the weight matrix to identify significant clusters of high and low crime rates.

### 7. **Variable exploration**

(Done by: Jeffery)

Visualisation of the distribution of various variables, filtering and standardizing data, and calculating correlations between variables. By identifying significant relationships between variables, we can possibly identify potential causes of crime.

### 8. **Spatial Clustering Configurations**

(Done by: Jeffery)

To uncover underlying spatial patterns in crime data, we explored various clustering configurations. By experimenting with different clustering algorithms and parameter settings, we aimed to identify the optimal clustering scheme that best represents the spatial distribution of crime.

### 9. **Spatial Clustering Results**

(Done by: Jeffery)

Interactive cluster plots and subplots, were used to visualize and interpret the clustering patterns. By comparing different clustering techniques, users can identify the most effective approach for understanding the spatial relationships between crime events.
